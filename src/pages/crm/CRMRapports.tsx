...
export default function CRMRapports() {
  ...
  // TODO MISSION-05: Migrate report persistence from localStorage to Supabase.
  // A `report_definitions` table should be created with columns:
  //   id (uuid), tenant_id (uuid), name (text), sources (jsonb), selected_fields (jsonb),
  //   filters (jsonb), join_conditions (jsonb), created_by (text), created_at, updated_at.
  // This would enable cross-device access, collaboration, and proper multi-tenant isolation.
  useEffect(() => {
    try {
      const storageKey = tenant?.id ? `crm_reports_v2_${tenant.id}` : "crm_reports_v2";
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) {
          ...
          console.warn("CRM Reports: invalid data in localStorage, resetting");
          localStorage.removeItem(storageKey);
        }
      }
      // Migrate legacy key (no tenant isolation) to tenant-scoped key
      if (tenant?.id) {
        const legacyData = localStorage.getItem("crm_reports_v2");
        if (legacyData && !saved) {
          try {
            const legacyParsed = JSON.parse(legacyData);
            if (Array.isArray(legacyParsed) && legacyParsed.length > 0) {
              localStorage.setItem(storageKey, legacyData);
              setSavedReports(legacyParsed);
              localStorage.removeItem("crm_reports_v2");
            }
          } catch {
            // Legacy data corrupted, ignore
          }
        }
      }
    } catch (error) {
      console.error("CRM Reports: failed to load saved reports from localStorage", error);
    ...
    const loadCollaborateurs = async () => {
      const { data, error } = await supabase
        ...
        .limit(200);
      if (error) {
        console.error("CRM Reports: failed to load collaborateurs", error);
      }
  ...
  // Sauvegarder dans localStorage (tenant-scoped)
  const saveReportsToStorage = (reports: SavedReport[]) => {
    try {
      const storageKey = tenant?.id ? `crm_reports_v2_${tenant.id}` : "crm_reports_v2";
      const serialized = JSON.stringify(reports);
      // Guard against localStorage quota (~5MB). Warn if approaching 1MB.
      if (serialized.length > 1_000_000) {
        console.warn(`CRM Reports: localStorage payload is large (${(serialized.length / 1024).toFixed(0)}KB). Consider migrating to Supabase.`);
      }
      localStorage.setItem(storageKey, serialized);
      setSavedReports(reports);
    } catch (error) {
      console.error("CRM Reports: failed to save reports to localStorage", error);
      // Likely QuotaExceededError
      toast.error(t('reports.errorExecuting'));
      // Still update state so the UI reflects the change this session
      setSavedReports(reports);
    }
  };
  ...
  const executeReport = async (report: SavedReport) => {
    ...
    try {
      ...
      if (report.sources.includes("contrats")) {
        const { data, error } = await supabase.from("policies").select("*").limit(1000);
      ...
      if (report.sources.includes("suivis")) {
        const { data, error } = await supabase.from("suivis").select("*").limit(1000);
