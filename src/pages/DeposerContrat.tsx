...
// ── Zod schemas for form validation (FE-05) ──

const sanaSchema = z.object({
...
const medioSchema = z.object({
  preneurNom: z.string().min(1, "Nom requis"),
  preneurPrenom: z.string().min(1, "Prénom requis"),
  preneurEmail: z.string().min(1, "Email requis").email("Email invalide"),
  preneurTel: z.string().min(1, "Téléphone requis"),
  confirmDocuments: z.literal(true, { errorMap: () => ({ message: "Veuillez confirmer les documents" }) }),
});

const businessSchema = z.object({
  entrepriseNom: z.string().min(1, "Nom d'entreprise requis"),
  chefNom: z.string().min(1, "Nom du chef d'entreprise requis"),
...
export default function DeposerContrat() {
  ...
  useEffect(() => {
    if (!isPartnerVerified && verificationStep !== 'email') {
  ...
  /** Extract first error per field from a Zod validation result */
  const extractZodErrors = (result: z.SafeParseError<unknown>): FormErrors => {
    const errors: FormErrors = {};
  ...
  const handleSubmitSana = async () => {
    setFormErrors({});
    const validation = sanaSchema.safeParse(sanaForm);
  ...
  const handleSubmitVita = async () => {
    setFormErrors({});
    const validation = vitaSchema.safeParse(vitaForm);
    if (!validation.success) {
      const errors = extractZodErrors(validation);
      setFormErrors(errors);
      toast({ title: t('common.error'), description: t('depositContract.fillRequired'), variant: "destructive" });
  ...
  const handleSubmitMedio = async () => {
    setFormErrors({});
    const validation = medioSchema.safeParse(medioForm);
    if (!validation.success) {
      const errors = extractZodErrors(validation);
      setFormErrors(errors);
      toast({ title: t('common.error'), description: t('depositContract.fillRequired'), variant: "destructive" });
  ...
  const handleSubmitBusiness = async () => {
    setFormErrors({});
    const validation = businessSchema.safeParse(businessForm);
    if (!validation.success) {
      const errors = extractZodErrors(validation);
      setFormErrors(errors);
      toast({ title: t('common.error'), description: t('depositContract.fillRequired'), variant: "destructive" });
        ...
        {selectedFormType === 'sana' && (
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={sanaForm.clientNom} onChange={(e) => { setSanaForm(prev => ({ ...prev, clientNom: e.target.value })); setFormErrors(prev => ({ ...prev, clientNom: undefined })); }} placeholder={t('depositContract.common.lastNamePlaceholder')} className={formErrors.clientNom ? 'border-destructive' : ''} />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={sanaForm.clientPrenom} onChange={(e) => { setSanaForm(prev => ({ ...prev, clientPrenom: e.target.value })); setFormErrors(prev => ({ ...prev, clientPrenom: undefined })); }} placeholder={t('depositContract.common.firstNamePlaceholder')} className={formErrors.clientPrenom ? 'border-destructive' : ''} />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="email" value={sanaForm.clientEmail} onChange={(e) => { setSanaForm(prev => ({ ...prev, clientEmail: e.target.value })); setFormErrors(prev => ({ ...prev, clientEmail: undefined })); }} placeholder="email@exemple.com" className={formErrors.clientEmail ? 'border-destructive' : ''} />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={sanaForm.clientTel} onChange={(e) => { setSanaForm(prev => ({ ...prev, clientTel: e.target.value })); setFormErrors(prev => ({ ...prev, clientTel: undefined })); }} placeholder="+41 79 000 00 00" className={formErrors.clientTel ? 'border-destructive' : ''} />
              ...
              <div className="space-y-2">
                <div className={`flex items-start gap-3 p-4 bg-muted/50 rounded-lg ${formErrors.confirmDocuments ? 'ring-2 ring-destructive' : ''}`}>
                  <Checkbox id="sana-confirm" checked={sanaForm.confirmDocuments} onCheckedChange={(c) => { setSanaForm(prev => ({ ...prev, confirmDocuments: c as boolean })); setFormErrors(prev => ({ ...prev, confirmDocuments: undefined })); }} />
                  <div className="space-y-1">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.lastName')} *</Label>
                    <Input value={vitaForm.clientNom} onChange={(e) => { setVitaForm(prev => ({ ...prev, clientNom: e.target.value })); setFormErrors(prev => ({ ...prev, clientNom: undefined })); }} placeholder={t('depositContract.common.lastNamePlaceholder')} className={formErrors.clientNom ? 'border-destructive' : ''} />
                    {formErrors.clientNom && <p className="text-sm text-destructive">{formErrors.clientNom}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.firstName')} *</Label>
                    <Input value={vitaForm.clientPrenom} onChange={(e) => { setVitaForm(prev => ({ ...prev, clientPrenom: e.target.value })); setFormErrors(prev => ({ ...prev, clientPrenom: undefined })); }} placeholder={t('depositContract.common.firstNamePlaceholder')} className={formErrors.clientPrenom ? 'border-destructive' : ''} />
                    {formErrors.clientPrenom && <p className="text-sm text-destructive">{formErrors.clientPrenom}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.email')} *</Label>
                    <Input type="email" value={vitaForm.clientEmail} onChange={(e) => { setVitaForm(prev => ({ ...prev, clientEmail: e.target.value })); setFormErrors(prev => ({ ...prev, clientEmail: undefined })); }} placeholder="email@exemple.com" className={formErrors.clientEmail ? 'border-destructive' : ''} />
                    {formErrors.clientEmail && <p className="text-sm text-destructive">{formErrors.clientEmail}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.phone')} *</Label>
                    <Input value={vitaForm.clientTel} onChange={(e) => { setVitaForm(prev => ({ ...prev, clientTel: e.target.value })); setFormErrors(prev => ({ ...prev, clientTel: undefined })); }} placeholder="+41 79 000 00 00" className={formErrors.clientTel ? 'border-destructive' : ''} />
                    {formErrors.clientTel && <p className="text-sm text-destructive">{formErrors.clientTel}</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className={`flex items-start gap-3 p-4 bg-muted/50 rounded-lg ${formErrors.confirmDocuments ? 'ring-2 ring-destructive' : ''}`}>
                  <Checkbox id="vita-confirm" checked={vitaForm.confirmDocuments} onCheckedChange={(c) => { setVitaForm(prev => ({ ...prev, confirmDocuments: c as boolean })); setFormErrors(prev => ({ ...prev, confirmDocuments: undefined })); }} />
                  <div className="space-y-1">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.lastName')} *</Label>
                    <Input value={medioForm.preneurNom} onChange={(e) => { setMedioForm(prev => ({ ...prev, preneurNom: e.target.value })); setFormErrors(prev => ({ ...prev, preneurNom: undefined })); }} placeholder={t('depositContract.common.lastNamePlaceholder')} className={formErrors.preneurNom ? 'border-destructive' : ''} />
                    {formErrors.preneurNom && <p className="text-sm text-destructive">{formErrors.preneurNom}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.firstName')} *</Label>
                    <Input value={medioForm.preneurPrenom} onChange={(e) => { setMedioForm(prev => ({ ...prev, preneurPrenom: e.target.value })); setFormErrors(prev => ({ ...prev, preneurPrenom: undefined })); }} placeholder={t('depositContract.common.firstNamePlaceholder')} className={formErrors.preneurPrenom ? 'border-destructive' : ''} />
                    {formErrors.preneurPrenom && <p className="text-sm text-destructive">{formErrors.preneurPrenom}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.email')} *</Label>
                    <Input type="email" value={medioForm.preneurEmail} onChange={(e) => { setMedioForm(prev => ({ ...prev, preneurEmail: e.target.value })); setFormErrors(prev => ({ ...prev, preneurEmail: undefined })); }} placeholder="email@exemple.com" className={formErrors.preneurEmail ? 'border-destructive' : ''} />
                    {formErrors.preneurEmail && <p className="text-sm text-destructive">{formErrors.preneurEmail}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.phone')} *</Label>
                    <Input value={medioForm.preneurTel} onChange={(e) => { setMedioForm(prev => ({ ...prev, preneurTel: e.target.value })); setFormErrors(prev => ({ ...prev, preneurTel: undefined })); }} placeholder="+41 79 000 00 00" className={formErrors.preneurTel ? 'border-destructive' : ''} />
                    {formErrors.preneurTel && <p className="text-sm text-destructive">{formErrors.preneurTel}</p>}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className={`flex items-start gap-3 p-4 bg-muted/50 rounded-lg ${formErrors.confirmDocuments ? 'ring-2 ring-destructive' : ''}`}>
                  <Checkbox id="medio-confirm" checked={medioForm.confirmDocuments} onCheckedChange={(c) => { setMedioForm(prev => ({ ...prev, confirmDocuments: c as boolean })); setFormErrors(prev => ({ ...prev, confirmDocuments: undefined })); }} />
                  <div className="space-y-1">
            ...
            <CardContent className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={businessForm.entrepriseNom} onChange={(e) => { setBusinessForm(prev => ({ ...prev, entrepriseNom: e.target.value })); setFormErrors(prev => ({ ...prev, entrepriseNom: undefined })); }} placeholder={t('depositContract.business.companyNamePlaceholder')} className={formErrors.entrepriseNom ? 'border-destructive' : ''} />
                    {formErrors.entrepriseNom && <p className="text-sm text-destructive">{formErrors.entrepriseNom}</p>}
                  </div>
                  <div className="space-y-2">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.lastName')} *</Label>
                    <Input value={businessForm.chefNom} onChange={(e) => { setBusinessForm(prev => ({ ...prev, chefNom: e.target.value })); setFormErrors(prev => ({ ...prev, chefNom: undefined })); }} className={formErrors.chefNom ? 'border-destructive' : ''} />
                    {formErrors.chefNom && <p className="text-sm text-destructive">{formErrors.chefNom}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label>{t('depositContract.common.birthDate')}</Label>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="email" value={businessForm.emailRetour} onChange={(e) => { setBusinessForm(prev => ({ ...prev, emailRetour: e.target.value })); setFormErrors(prev => ({ ...prev, emailRetour: undefined })); }} placeholder="email@entreprise.ch" className={formErrors.emailRetour ? 'border-destructive' : ''} />
                    {formErrors.emailRetour && <p className="text-sm text-destructive">{formErrors.emailRetour}</p>}
                  </div>
                  <div className="space-y-2">
