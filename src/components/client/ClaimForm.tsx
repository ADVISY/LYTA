...
const claimSchema = z.object({
  claim_type: z.enum(['auto', 'sante', 'menage', 'juridique', 'autre'], {
    required_error: "Le type de sinistre est requis",
  }),
  policy_id: z.string().optional(),
  incident_date: z.string().min(1, "La date de l'incident est requise"),
  description: z.string().min(10, "La description doit contenir au moins 10 caractères"),
  amount_claimed: z.union([
    z.literal(''),
    z.string().refine(
...
export default function ClaimForm({ clientId, onSuccess, onCancel }: ClaimFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (clientError) {
        console.error('Error fetching client tenant:', clientError);
      }

      // tenant_id is critical for proper claim routing — warn if missing
      const tenantId = clientData?.tenant_id || null;
      if (!tenantId) {
        console.warn('ClaimForm: tenant_id is null for client', clientId);
      }
      ...
      if (uploadedFiles.length > 0 && claim) {
        ...
        for (const file of uploadedFiles) {
          ...
          const fileKey = `claims/${claim.id}/${Date.now()}-${sanitizedName}`;
          ...
          if (doc) {
            ...
            if (linkError) {
              console.error('Claim document link error:', linkError);
            }
          }
        }
      }
