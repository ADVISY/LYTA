...
export function useScanBatches() {
  ...
  const {
    ...
    isError,
    refetch,
  } = usePaginatedQuery<ScanBatch>({
    queryKey: ['scan_batches', tenantId ?? ''],
    buildQuery: (client) =>
      client.from('scan_batches')
        .select(`
          *,
  ...
  const error = isError ? 'Erreur lors du chargement des dossiers' : null;
  ...
  const deleteBatch = async (batchId: string): Promise<boolean> => {
    try {
      ...
      if (docs && docs.length > 0) {
        const filePaths = docs.map((d) => d.file_key);
