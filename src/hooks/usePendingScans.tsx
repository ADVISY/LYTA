...
export function usePendingScans() {
  ...
  const { data: totalCount = 0 } = useQuery({
    queryKey: ['pending_scans', tenantId ?? '', 'count'],
    queryFn: async () => {
  ...
  // Main data query with enrichment
  const { data: scans = [], isLoading: loading, isError, refetch } = useQuery({
    queryKey: ['pending_scans', tenantId ?? '', 'page', page, PAGE_SIZE],
    queryFn: async () => {
      ...
      const scansWithFields: PendingScan[] = (scanData || []).map((scan: any) => {
        const scanAudit = auditData.find((a: any) => a.scan_id === scan.id);
        ...
        return {
          ...
          fields: fieldsData
            .filter((f: any) => f.scan_id === scan.id)
            .map((f: any) => ({
              id: f.id,
      ...
      return scansWithFields;
    },
    enabled: !!tenantId,
  });

  const error = isError ? 'Erreur lors du chargement des scans' : null;
