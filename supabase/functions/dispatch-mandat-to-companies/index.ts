...
function renderEmailHtml(opts: {
  ...
  /** Reference number to put at the top of the letter (auto-generated). */
  reference: string;
  /** Localised "Lausanne, le 10 mai 2026" — date string for header. */
  dateLine: string;
}): string {
  ...
  // Tenant primary color is honoured for the accent bar; falls back to
  // a sober slate-blue that reads professional on every email client.
  const accent = (opts.primaryColor && /^#[0-9a-fA-F]{3,8}$/.test(opts.primaryColor))
    ? opts.primaryColor
    : "#1e3a8a";

  // Inline-only CSS — most enterprise insurance webmail clients (Outlook,
  // Lotus Notes, IBM Verse) strip <style> tags. Keep it sober: no gradients,
  // no shadows, monospace-friendly typography. This is a formal letter.
  return `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="utf-8">
        ...
        <tr>
          <td style="padding:18px 36px 4px 36px;">
            <div style="border-top:1px solid #e1e5ea;padding-top:14px;font-size:11px;color:#6b7280;">
              ...
              mandat_${safeRef}.pdf — mandat de gestion signé électroniquement
 ...
 * companies often look at the visible sender name first ("Advisy Conseil
 * SA") rather than the email address — and we definitely don't want to
 * show "lyta.ch" there. Escape any quotes / backslashes in the cabinet
 * name before wrapping in double quotes.
 */
...
async function sendResendWithAttachment(opts: {
  ...
  const body = {
    // Use the cabinet name as visible sender so the recipient sees
    // "Advisy" (or whatever the tenant brand is) in their inbox, not
    // a generic "noreply@lyta.ch". The address itself is still the
    // shared LYTA mailbox so deliverability stays high.
...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch tenant branding so the email letterhead reflects the
    // cabinet's identity (primary color + phone + website + display
    // name). Failure is non-fatal — we just fall back to defaults.
    let tenantBranding: {
    ...
    try {
      const { data: brandingRow } = await admin
        ...
        .select("display_name, primary_color, company_phone, company_website, company_address")
        .eq("tenant_id", mandat.tenant_id)
        .maybeSingle();
      if (brandingRow) tenantBranding = brandingRow as typeof tenantBranding;
    } catch (_) {
      // Ignore — defaults below
    }
    const cabinetDisplayName =
      (tenantBranding.display_name && tenantBranding.display_name.trim()) ||
      cabinetName;
    const cabinetPhone = tenantBranding.company_phone || null;
    const cabinetWebsite = tenantBranding.company_website || null;
    const cabinetPrimaryColor = tenantBranding.primary_color || null;
    // Letter date in Swiss French format
    const dateLine = (() => {
      const d = new Date();
      const months = [
    ...
    // Short reference based on the signature_request id (last 6 hex chars)
    ...
    for (const rawName of uniqueNames) {
      ...
      const subject = `Mandat de gestion — ${clientFullName} — ${cabinetDisplayName}`;

      let resendId: string | undefined;
      let sendError: string | null = null;
      try {
        const r = await sendResendWithAttachment({
          fromName: cabinetDisplayName,
