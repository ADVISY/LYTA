...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (type === "mandat_signed" && !data?.temporaryPassword) {
      ...
      // Targeted user lookup instead of listing all users (avoids OOM)
      ...
      if (existingProfile) {
        ...
        if (authUser && authUser.email?.toLowerCase() === clientEmail.toLowerCase()) {
    ...
    log.error("Error in send-crm-email function", { error: error instanceof Error ? error.message : String(error) });
