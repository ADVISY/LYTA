...
const log = createLogger("send-client-notification-email");
...
const handler = async (req: Request): Promise<Response> => {
  log.info("send-client-notification-email invoked");
  ...
  try {
    ...
    await checkRateLimit(req, "send-client-notification-email", 15);
    ...
    log.info(`Processing notification email`, { clientId: client_id, kind: notification_kind });
    ...
    if (clientError || !client) {
      log.error("Client not found", { error: clientError });
      throw new Error("Client not found");
    }

    if (!client.email) {
      log.info("Client has no email, skipping notification");
    ...
    const { fromAddress, senderEmail, senderName } = getSenderAddress(tenantBranding, "Votre cabinet");

    log.info(`Sending email`, { to: client.email, from: fromAddress });
    ...
    log.error("Error sending notification email", { error: errorMessage });
