...
serve(async (req) => {
  ...
  try {
    ...
    log.info("King user verified", { userId: user.id });
    ...
    log.info("Activating tenant", { tenant_id });
    ...
    // Check if already activated (idempotent) — check both columns for backward compat
    if ((tenant.tenant_status === 'active' || tenant.status === 'active') && tenant.activated_at) {
      log.info("Tenant already activated", { tenant_id });
    ...
    if (!adminUserId) {
      ...
      log.info("Admin user created", { userId: adminUserId });
    ...
    if (resetError) {
      log.warn("Warning: Could not generate reset link", { error: resetError.message });
    }
    ...
    if (resetData?.properties?.action_link) {
      try {
        ...
        log.warn("Warning: Failed to send email", { error: emailError instanceof Error ? emailError.message : emailError });
