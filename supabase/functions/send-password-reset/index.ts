...
const log = createLogger("send-password-reset");
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    await checkRateLimit(req, "send-password-reset", 3);
    ...
    // Find the user by email (targeted lookup instead of listing all users)
    ...
    if (profileMatch) {
      const { data: { user: authUser }, error: userError } = await supabaseAdmin.auth.admin.getUserById(profileMatch.id);
      if (userError) {
        log.error("Error fetching user by id", { error: userError });
    ...
    if (!user) {
      ...
      log.info("User not found, returning success anyway for security");
      return new Response(
    ...
    if (clientRow?.tenant_id) {
      ...
      log.info("Tenant resolved from clients table", { tenantId });
    }
    ...
    if (!tenantId) {
      ...
      if (assignment?.tenant_id) {
        ...
        log.info("Tenant resolved from user_tenant_assignments", { tenantId });
      }
    }
    ...
    if (tenantId) {
      ...
      if (tenant) {
        ...
        if (tb) {
          ...
          log.info("Branding loaded for tenant", { tenantName });
        }
      }
    } else {
    ...
    if (resetError) {
      log.error("Error generating reset link", { error: resetError });
    ...
    if (!resendResponse.ok) {
      log.error("Resend API error", { error: resendData });
      throw new Error(resendData.message || "Failed to send email");
    }

    log.info("Password reset email sent successfully", { emailId: resendData.id });
