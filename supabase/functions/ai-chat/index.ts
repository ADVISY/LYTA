...
serve(async (req) => {
  ...
  try {
    ...
    log.info('AI Chat request', { conversationId, sessionId, userType, messageCount: messages?.length, ip: clientIp });

    // Validate sessionId
    if (!sessionId || typeof sessionId !== 'string' || sessionId.length < 10) {
      log.error('Invalid sessionId provided');
      return new Response(
    ...
    if (!rateLimitResult.allowed) {
      log.warn('Rate limit exceeded', { sessionId, ip: clientIp });
      return new Response(
        JSON.stringify({ 
    ...
    log.error('Error in ai-chat function', { error: error instanceof Error ? error.message : String(error) });
